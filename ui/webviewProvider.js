...
class MyWebviewProvider {
  ...
  resolveWebviewView(webviewView) {
    ...
    const iconPathOnDisk = vscode.Uri.file(
      path.join(extensionPath, "media", "icon.svg")
    );
    ...
    try {
      if (this.router && typeof this.router.listModes === "function") {
        this.router
          .listModes()
          .then((modesList) => {
            const modesArr = Array.isArray(modesList)
              ? modesList
              : modesList && modesList.length
              ? modesList
              : [];
            const taglines = {};
            for (const m of modesArr) {
              try {
                if (m && m.id)
                  taglines[m.id] =
                    m.tagline || (m.wrappers && m.wrappers.title) || "";
              } catch {}
            }
            if (this.webviewView && this.webviewView.webview) {
              this.webviewView.webview.postMessage({
                command: "modesTaglines",
                taglines,
              });
            }
          })
        ...
        switch (message.command) {
          case "newChat":
            try {
              ...
              if (this.webviewView) {
                ...
                const iconUri =
                  this.webviewView.webview.asWebviewUri(iconPathOnDisk);
                this.webviewView.webview.postMessage({
                  command: "placeholderResources",
                  iconUri: String(iconUri),
                });
          ...
          case "sendPrompt":
            try {
              ...
              if (this.isWaitingForUserInput) {
                console.log(
                  "Ignoring sendPrompt request - waiting for user input"
                );
                this.webviewView.webview.postMessage({
              ...
              if (modeId === "duality") {
                ...
                if (primaryModelId && secondaryModelId) {
                  ...
                  throw new Error(
                    "Primary and secondary model IDs are required for Duality mode"
                  );
                }
                await this.handleDualityMode(
                  modelId,
                  prompt,
                  requestId,
                  message
              ...
              if (modeId && typeof this.router.runMode === "function") {
                ...
                try {
                  ...
                  if (
                    ...
                    resp = await this.router.sendPromptNvidia(
                      modelId,
                      prompt,
                      modeId,
                      requestId
                    ...
                    resp = await this.router.sendPrompt(
                      modelId,
                      prompt,
                      modeId,
                      requestId
              ...
              try {
                ...
                responseForUI.plain_text =
                  resp &&
                  typeof resp.user_text === "string" &&
                  resp.user_text.length
                    ? resp.user_text
                    : parsed.plain_text;
          ...
          case "setAutoPilotTerminalExecution":
            try {
              ...
              try {
                ...
                await vscodeCfg.workspace
                  .getConfiguration("vsx")
                  .update(
                    "autopilot.runTerminalCommands",
                    enabled,
                    vscodeCfg.ConfigurationTarget.Global
                  );
                try {
                  // Show a brief info message to confirm the change
                  vscodeCfg.window.showInformationMessage(
                    `Autopilot terminal execution ${
                ...
                console.error(
                  "Failed to update autopilot.runTerminalCommands setting",
                  err
                );
              }
              if (this.webviewView && this.webviewView.webview)
                this.webviewView.webview.postMessage({
                  command: "autoPilotTerminalChanged",
                  enabled,
                });
            } catch (e) {
          ...
          case "getAutoPilotTerminalExecution":
            try {
              const enabled = Boolean(
                vscode.workspace
                  .getConfiguration("vsx")
                  .get("autopilot.runTerminalCommands") || false
              );
              if (this.webviewView && this.webviewView.webview)
                this.webviewView.webview.postMessage({
                  command: "autoPilotTerminalChanged",
          ...
          case "runTerminalCommand":
            try {
              ...
              const requestId =
                message.requestId ||
                (toolCall && toolCall.requestId) ||
                String(Date.now());
              const cmd =
                toolCall.args && (toolCall.args.command || toolCall.args.cmd)
                  ? toolCall.args.command || toolCall.args.cmd
                  : "";
              let cwd =
                toolCall.args && toolCall.args.cwd
                  ? toolCall.args.cwd
                  : this.getWorkspaceRoot(); // Default to workspace root

              if (!cmd) {
                if (this.webviewView && this.webviewView.webview) {
                  this.webviewView.webview.postMessage({
                    command: "terminalCommandResult",
                    requestId,
              ...
              const execProcess = child_process.exec(
                cmd,
                {
                  cwd: cwd,
                  windowsHide: true,
                ...
                (err, stdout, stderr) => {
                  ...
                  const outputText = combined.length
                    ? combined
                    : err
                  ...
                  try {
                    ...
                    console.error("Failed to send terminal command result", e);
                  }
                }
              );
              ...
              try {
                ...
                console.error("Failed to send terminal command error", err);
              }
            }
            return;
          case "skipTerminalCommand":
            try {
              const requestId =
                message.requestId ||
                (message.toolCall && message.toolCall.requestId) ||
                String(Date.now());
              try {
                this.webviewView.webview.postMessage({
                  command: "terminalCommandSkipped",
                  requestId,
                });
              } catch (e) {}
            ...
            return;
          case "setWaitingForUserInput":
            try {
              // Track waiting state to prevent new requests
              this.isWaitingForUserInput = message.waiting;
              this.waitingRequestId = message.waiting
                ? message.requestId
                : null;
              console.log(
                `Set waiting for user input: ${message.waiting} (requestId: ${message.requestId})`
              );
            } catch (e) {
              console.error("Error handling setWaitingForUserInput", e);
            }
            return;
          ...
          case "tool.writeFile":
            ...
            try {
              await this.handleWriteFileTool(
                message.filePath,
                message.newContent,
                message.requestId
              );
              ...
              const retryKey = `writeFile_${message.requestId}`;
              const currentRetries = this.toolRetries?.get(retryKey) || 0;
              ...
              if (currentRetries < 3) {
                ...
                this.toolRetries.set(retryKey, currentRetries + 1);

                // Notify webview of retry attempt
                this.webviewView.webview.postMessage({
                  command: "toolRetryAttempt",
                  requestId: message.requestId,
                  toolType: "writeFile",
                ...
                setTimeout(async () => {
                  try {
                    ...
                    console.error("Retry handleWriteFileTool failed", retryErr);
                    this.webviewView.webview.postMessage({
          ...
          case "acceptFileEdit":
            try {
              await this.handleAcceptFileEdit(
                message.filePath,
                message.changes,
                message.requestId,
                message.command === "autoAcceptFileEdit"
              );
            } catch (err) {
          ...
          case "rejectFileEdit":
            try {
              await this.handleRejectFileEdit(
                message.filePath,
                message.requestId
              );
            } catch (err) {
        ...
        if (selectedClient.client === "cerebras") {
          const r = await vscode.window.showQuickPick(
            [
              {
                label: "Low",
                description: "Lower reasoning effort (faster)",
                value: "low",
              },
              {
                label: "Medium",
              ...
              {
                label: "High",
                description: "High reasoning effort (slower)",
                value: "high",
              },
            ],
  ...
  async promptSetCerebrasReasoning() {
    try {
      ...
      vscode.window.showInformationMessage(
        `Cerebras reasoning effort set to ${choice.label}`
      );
      if (this.webviewView)
        this.webviewView.webview.postMessage({
          command: "cerebrasReasoningChanged",
          value: choice.value,
        });
    } catch {}
  }
        ...
        [
          {
            ...
            value: false,
            description:
              "Let the primary model decide between direct execution or subtasks",
          },
          {
            label: "Force Subtasks",
            value: true,
            description:
              "Always create subtasks without asking the primary model to decide",
          },
        ],
        {
  ...
  async handleLegacyMode(modelId, prompt, requestId, message) {
    try {
      ...
      try {
        if (message && Array.isArray(message.previous_chat_history))
          prevHistory = message.previous_chat_history;
      } catch {}
      ...
      if (resp && resp.tools_called && Array.isArray(resp.tools_called)) {
        const terminalCommands = resp.tools_called.filter(
          (tool) =>
            tool.requiresConfirmation &&
            (tool.tool === "terminal_command" ||
              tool.tool === "terminalcommand" ||
        ...
        if (terminalCommands.length > 0) {
          ...
          try {
            await this.handleLegacyModeTerminalConfirmations(
              resp,
              terminalCommands,
              sessionContext
            );
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            if (maybeJson && typeof maybeJson.user_text === "string")
              user_text = maybeJson.user_text;
          } catch {
            // not JSON
          }
        }
        ...
        try {
          if (
            !responseForUI.text ||
            String(responseForUI.text).trim().length === 0
          ) {
            if (
              typeof responseForUI.plain_text === "string" &&
              responseForUI.plain_text.trim().length
            ) {
              responseForUI.text = responseForUI.plain_text;
            } else if (
              typeof responseForUI.user_text === "string" &&
              responseForUI.user_text.trim().length
            ) {
              responseForUI.text = responseForUI.user_text;
            } else if (responseForUI.raw !== undefined) {
              try {
                responseForUI.text =
                  typeof responseForUI.raw === "string"
                    ? responseForUI.raw
                    : JSON.stringify(responseForUI.raw);
              } catch {
          ...
          try {
            responseForUI.text =
              responseForUI.plain_text || responseForUI.user_text || "";
          } catch (e) {
            responseForUI.text = "";
          }
        }
      ...
      try {
        ...
        console.error(
          "Failed to post final promptResponse for legacy mode",
          err
        );
      }
  ...
  handleLegacyModeConfirmation(message) {
    try {
      ...
      console.log("Legacy Mode confirmation received:", {
        todoId,
        approved,
        feedback,
      });
      ...
      for (const session of this.legacyModeSessions.values()) {
        if (
          session.confirmationCallbacks &&
          session.confirmationCallbacks.has(todoId)
        ) {
  ...
  async handleLegacyModeTerminalConfirmations(
    resp,
    terminalCommands,
    sessionContext
  ) {
    try {
      ...
      try {
        ...
        if (autoRun) {
          console.log(
            "Autopilot is allowed to run terminal commands automatically; executing commands"
          );
          ...
          for (const tc of terminalCommands) {
            try {
              ...
              confirmationResults.push({
                ...r,
                approved: true,
                todoId: `terminal_${sessionContext.requestId}_${confirmationResults.length}`,
              });
            } catch (err) {
              confirmationResults.push({
          ...
          for (
            let i = 0;
            i < updatedToolsCalled.length &&
            resultIndex < confirmationResults.length;
            i++
          ) {
            const t = updatedToolsCalled[i];
            if (
              t.requiresConfirmation &&
              (t.tool === "terminal_command" ||
                t.tool === "terminalcommand" ||
                t.tool === "execute_command")
            ) {
              updatedToolsCalled[i] = {
                ...t,
                ...confirmationResults[resultIndex],
        ...
        console.error(
          "Error checking autopilot.runTerminalCommands setting",
          err
        );
      }
      ...
      const confirmationPromises = terminalCommands.map(
        async (terminalCommand, index) => {
          ...
          return new Promise((resolve) => {
            ...
            sessionContext.confirmationCallbacks.set(
              todoId,
              async ({ approved }) => {
      ...
      confirmationResults.forEach((result) => {
        const terminalCommandIndex = updatedToolsCalled.findIndex(
          (tool) =>
            tool.requiresConfirmation &&
            (tool.tool === "terminal_command" ||
              tool.tool === "terminalcommand" ||
  ...
  async handleDualityMode(modelId, prompt, requestId, message) {
    try {
      ...
      try {
        ...
        console.error(
          "Failed to post final promptResponse for duality mode:",
          err
        );
      }
    } catch (err) {
      ...
      try {
        ...
        console.error(
          "Failed to post error response for duality mode:",
          postErr
        );
      }
    }
  ...
  handleDualityModeProgressUpdate(message) {
    try {
      ...
      console.log("Duality Mode progress update received:", {
        requestId,
        progressData,
      });
  ...
  handleDualityModeSubtaskUpdate(message) {
    try {
      ...
      console.log("Duality Mode subtask update received:", {
        requestId,
        subtaskIndex,
        status,
      });
    ...
    try {
      ...
      switch (toolName) {
        ...
        case "deleteFile":
          ...
          break;
        case "searchFiles":
          result = await this.legacySearchFiles(
            params.pattern,
            params.excludePattern
          );
          break;
        case "executeCommand":
        ...
        case "executeTerminal":
          result = await this.legacyExecuteTerminal(
            params.command,
  ...
  async handleWriteFileTool(filePath, newContent, requestId) {
    try {
      ...
      const originalContent = originalExists
      ...
      try {
        const vscode = require("vscode");
        autopilot = Boolean(
          vscode.workspace.getConfiguration("vsx").get("autopilot.enabled")
        );
      } catch {}
      ...
      if (autopilot) {
        try {
          ...
          try {
            require("fs").mkdirSync(require("path").dirname(resolvedPath), {
              recursive: true,
            });
          } catch {}
      ...
      const keep = await this.showDiffAndPrompt(
        originalContent,
        newContent || "",
        resolvedPath,
        requestId
  ...
  async showDiffAndPrompt(originalContent, proposedContent, targetPath) {
    ...
    try {
      ...
      for (const p of parts) {
        ...
        if (segLines.length > 0 && segLines[segLines.length - 1] === "")
          segLines.pop();
          ...
          for (const ve of visible) {
            try {
              if (
                ve.document &&
                ve.document.uri &&
                ve.document.uri.fsPath &&
                require("path").resolve(ve.document.uri.fsPath) === absPath
              ) {
                editor = ve;
                doc = ve.document;
                break;
      ...
      await editor.edit((eb) => {
        ...
        eb.replace(
          new vscode.Range(new vscode.Position(0, 0), lastLine),
          diffText
        );
      });
      ...
      try {
        editor.setDecorations(addedDeco, addedRanges);
      } catch {}
      try {
      ...
      try {
        const keepItem = vscode.window.createStatusBarItem(
          vscode.StatusBarAlignment.Right,
          100
        );
        keepItem.text = "$(check) Keep";
        keepItem.command = {
          command: "vsx.keepChange",
          title: "Keep change",
          arguments: [id],
        };
        ...
        const undoItem = vscode.window.createStatusBarItem(
          vscode.StatusBarAlignment.Right,
          99
        );
        undoItem.text = "$(close) Undo";
        undoItem.command = {
          command: "vsx.undoChange",
          title: "Undo change",
          arguments: [id],
        };
        ...
        try {
          if (this.context && this.context.subscriptions) {
            this.context.subscriptions.push(keepItem);
            this.context.subscriptions.push(undoItem);
          }
        } catch {}
      } catch (e) {
        console.error("status bar creation failed", e);
      }
      ...
      try {
        ...
        const md = new vscode.MarkdownString(
          `[Keep](command:vsx.keepChange?${encodeURIComponent(
            JSON.stringify([id])
          )}) ⠀ [Undo](command:vsx.undoChange?${encodeURIComponent(
            JSON.stringify([id])
          )})`
        );
        md.isTrusted = true;
        const hoverDeco = vscode.window.createTextEditorDecorationType({
          after: { contentText: " ⠀ ⠀(Keep / Undo) ", margin: "0 0 0 20px" },
        });
        try {
          editor.setDecorations(hoverDeco, [
            { range: hoverRange, hoverMessage: md },
          ]);
        } catch (e) {
          console.error("set hover decoration failed", e);
        }
        pEntry.hoverDeco = hoverDeco;
      } catch (e) {
        console.error("hover creation failed", e);
      }
      ...
      try {
        try {
          vscode.window.showInformationMessage(
            `Preview opened for ${targetPath}. Use status bar Keep/Undo.`
          );
        } catch (e) {}
      } catch (e) {}
  ...
  async handleKeepById(id) {
    try {
      if (!this.pendingWriteProposals || !this.pendingWriteProposals.has(id))
        return false;
      ...
      const path = require("path");
      try {
        fs.mkdirSync(path.dirname(p.targetPath), { recursive: true });
      } catch {}
      ...
      try {
        await p.editor.edit((eb) => {
          const last =
            p.editor.document.lineCount > 0
              ? p.editor.document.lineAt(p.editor.document.lineCount - 1).range
                  .end
              : new vscode.Position(0, 0);
          eb.replace(
            new vscode.Range(new vscode.Position(0, 0), last),
            p.proposedContent || ""
          );
        });
        try {
          await p.editor.document.save();
        } catch {}
      } catch {}

      // cleanup decorations and status bar
      try {
        p.editor.setDecorations(p.addedDeco, []);
      } catch {}
      try {
        p.editor.setDecorations(p.removedDeco, []);
      } catch {}
      try {
        p.addedDeco.dispose();
      } catch {}
      try {
        p.removedDeco.dispose();
      } catch {}
      try {
        p.hoverDeco && p.hoverDeco.dispose();
      } catch {}
      try {
        p.keepItem && p.keepItem.dispose();
      } catch {}
      try {
        p.undoItem && p.undoItem.dispose();
      } catch {}
  ...
  cleanupExpiredSessions() {
    try {
      ...
      for (const [requestId, session] of this.dualityModeSessions.entries()) {
        if (
          session.startTime &&
          now - new Date(session.startTime).getTime() > maxAge
        ) {
          this.dualityModeSessions.delete(requestId);
      ...
      try {
        ...
        if (dualityMode.SessionManager) {
          const activeSessions =
            dualityMode.SessionManager.getAllActiveSessions();
    ...
    try {
      ...
      if (dualityMode.SessionManager) {
        const recoveredSession =
          dualityMode.SessionManager.recoverSession(requestId);
        if (recoveredSession) {
